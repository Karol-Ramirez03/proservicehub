...
-- DELIMITER $$
-- DROP TRIGGER IF EXISTS after_update_orden_servicio$$
...
--     IF NEW.estado_orden_servicio_id = 1 THEN
...
BEGIN
	...
	IF NEW.id_empleado IS NOT NULL THEN
		SET NEW.estado_orden_servicio_id = 2;
        SET id_empleado_asignado = NEW.id_empleado;
        
        SELECT numeroordent + 1 INTO numeroSiguiente
        FROM orden_Trabajo
        ORDER BY numero_orden_trabajo DESC
        LIMIT 1;
